...
static void register_device(const char* dev_name, const char* dev_path, unsigned char ep_in,
                            ...
                            unsigned zero_mask, size_t max_packet_size, int fd) {
...
void usb_init(int fd) {
    D("usb_init with device fd:%d", fd);

    std::string dev_name;
    if (!android::base::Readlink("/proc/self/fd" + std::to_string(fd), &dev_name)) {
        D("Error: faiiled to readlink /proc/self/fd/%d", fd);
        unix_close(fd);
        return;
    } else {
        D("readpath: %s", dev_name.c_str());
    }
    ...
    while (bufptr < bufend) {
        ...
        if (type == USB_DT_INTERFACE) {
            ...
            if (interface->bNumEndpoints == 2 &&
                ...
                register_device(dev_name.c_str(), devpath, local_ep_in,
                                local_ep_out, interface->bInterfaceNumber,
                                device->iSerialNumber, zero_mask, max_packet_size, fd);
                break;
            }
        } else if (!length) {
