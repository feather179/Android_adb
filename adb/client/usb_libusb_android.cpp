...
struct LibusbConnection : public Connection {
    ...
    bool OpenDevice(std::string* error) {
        auto initHandle = [this, error]() -> bool {
        ...
        if (device_handle_) {
            ...
            // return false;
            VLOG(USB) << "device already opened";
        } else {
            if (!initHandle()) {
                LOG_ERR(error, "failed to init device handle");
                return false;
            }
        }

        for (uint8_t endpoint : {read_endpoint_, write_endpoint_}) {
            int rc = libusb_clear_halt(device_handle_.get(), endpoint);
            if (rc != 0) {
    ...
    static std::optional<std::shared_ptr<LibusbConnection>> Create(unique_device device, unique_device_handle handle = nullptr) {
        auto connection = std::make_unique<LibusbConnection>(std::move(device));
        if (!connection) {
            LOG(FATAL) << "failed to construct LibusbConnection";
        }

        connection->device_handle_ = std::move(handle);
        ...
        VLOG(USB) << "connection serial: " << connection->serial_;
#else
...
static void register_device(libusb_device *device_raw, libusb_device_handle *handle_raw) {
    std::string device_address = "usb:" + get_device_address(device_raw);
    VLOG(USB) << "device connected: " << device_address;

    unique_device device(libusb_ref_device(device_raw));
    unique_device_handle handle(handle_raw);
    auto connection_opt = LibusbConnection::Create(std::move(device), std::move(handle));
    if (!connection_opt) {
        LOG(FATAL) << "ffailed to create LibusbConnection";
        return;
    }
...
void usb_init(int fd) {
    VLOG(USB) << "initializing libusb with fd: " << fd;

    libusb_context *ctx = nullptr;
    libusb_device_handle *devh = nullptr;
    int r = 0;

    struct libusb_init_option options[] = { LIBUSB_OPTION_NO_DEVICE_DISCOVERY };
    r = libusb_init_context(nullptr, options, 1);
    if (r < 0) {
        LOG(FATAL) << "libusb_init failed: " << r;
        return;
    }
    r = libusb_wrap_sys_device(nullptr, (intptr_t)fd, &devh);
    if (r < 0) {
        LOG(FATAL) << "libusb_wrap_sys_device failed: " << r;
        return;
    } else if (devh == nullptr) {
        LOG(FATAL) << "libusb_wrap_sys_device returned invalid handle";
        return;
    }

    VLOG(USB) << "libusb init successfully";
    ...
    register_device(libusb_get_device(devh), devh);
