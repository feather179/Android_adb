...
std::string adb_version() {
    ...
    return android::base::StringPrintf(
            ...
            android::build::GetBuildNumber().c_str(), adb_get_executable_path().c_str(), // android::base::GetExecutablePath().c_str(),
            GetOSVersion().c_str());
}
...
int launch_server(const std::string& socket_spec, const char* one_device) {
    ...
    std::string basename = android::base::Basename(executable_path);
    std::string adb_path = "";
    ...
    std::vector<const char*> child_argv = {"adb"};

    if (basename == "linker" || basename == "linker64") {
        adb_path = adb_get_executable_path();
        child_argv.push_back(adb_path.c_str());
    }
    child_argv.push_back("-L");
    child_argv.push_back(socket_spec.c_str());
    ...
    if (pid == 0) {
        ...
        int result = execv(executable_path.c_str(), const_cast<char* const*>(child_argv.data()));
...
HostRequestResult handle_host_request(std::string_view service, TransportType type,
    ...
    if (service == "server-status") {
        ...
        status.set_executable_absolute_path(adb_get_executable_path());
